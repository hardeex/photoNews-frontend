...
class MisplaceAndFoundController extends Controller
{
    ...
    public function submitMisplaced(Request $request)
{
    Log::info('Misplaced item submission method is called...', [
    ...
    $apiUrl = config('api.base_url') . '/submit/misplaced-item';
    Log::info('Connecting to API URL for misplaced item creation', [
        'api_url' => $apiUrl,
    ]);
    ...
    try {
        ...
        if ($response->successful()) {
            Log::info('Misplaced item successfully created through external API', [
                'response_data' => $response->json(),
            ]);

            return redirect()->back()->with('success', 'Misplaced item created successfully!');
        } else {
        ...
        return back()->withErrors(['error' => 'An error occurred while submitting the misplaced item.']);
    }
}
...
public function submitFound(Request $request)
{
    ...
    $apiUrl = config('api.base_url') . '/submit/found-item';
