...
<body>
    ...
    <div class="exchange-rate fixed top-16 left-0 right-0 z-40">
        ...
        <div class="relative overflow-hidden bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 py-4">
            <div class="animate-ticker flex whitespace-nowrap">
                @php
                    $exchangeRates = [
                        ['flag' => '🇺🇸', 'pair' => 'USD/EUR', 'rate' => '0.85', 'color' => 'text-green-300'],
                        ['flag' => '🇺🇸', 'pair' => 'USD/GBP', 'rate' => '0.74', 'color' => 'text-yellow-300'],
                        ['flag' => '🇺🇸', 'pair' => 'USD/JPY', 'rate' => '110.21', 'color' => 'text-red-300'],
                        // ... (other rates)
                    ];
                @endphp

                @foreach ($exchangeRates as $rate)
                    <span class="mx-4 text-sm font-semibold {{ $rate['color'] }} flex items-center">
                        <span class="mr-2 text-lg">{{ $rate['flag'] }}</span>
                        {{ $rate['pair'] }}: {{ $rate['rate'] }}
                    </span>
                @endforeach
            </div>
